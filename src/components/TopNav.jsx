...
export default function TopNav({ onToggleMobileMenu }) {
  const { theme, toggleTheme, lang, toggleLanguage, t, user, logout, userRole, switchRole, currentSupplier } = useApp();
    ...
    <header className={`h-14 border-b px-4 sm:px-6 flex items-center justify-between sticky top-0 z-40 transition-colors ${
      ...
      <div className="flex items-center space-x-2 sm:space-x-2.5">
        ...
        <button
          ...
          className={`px-2.5 py-1.5 rounded-md border text-xs font-semibold transition flex items-center space-x-1.5 shadow-sm ${
            isSupplier
              ...
              : 'bg-blue-50 text-blue-800 border-blue-300 hover:bg-blue-100 dark:bg-blue-950/60 dark:text-blue-300 dark:border-blue-800'
          }`}
          title={isSupplier ? 'Switch to Kirana Retailer Portal' : 'Switch to Wholesale Supplier Portal'}
        >
          {isSupplier ? (
            <>
            ...
            <>
              <Store className="w-3.5 h-3.5 text-blue-700 dark:text-blue-400" />
              <span className="hidden sm:inline">Switch to Supplier</span>
            </>
          )}
        </button>
